...
void replay_mode() {
    uint8_t *entropies = (uint8_t*)malloc(32 * opts.maxkeys);
    generate_entropy(opts.entropy, opts.maxkeys, entropies);

    CudaState cl_state;
    init_cuda(&cl_state, opts.pipeline_opt);
    ...
    for(int i=0; i<opts.maxkeys; i++) {
        ...
        char *mnemonics_arr[] = {mnemonic};
        derive_seed_batch_cuda(&cl_state, mnemonics_arr, 1, seed);
        ...
        // bip44
        derive_bip32_cuda(&cl_state, MODE_BIP44, seed, 1, 1, child_priv);
        ...
        // bip49
        derive_bip32_cuda(&cl_state, MODE_BIP49, seed, 1, 1, child_priv);
        print_hex(child_priv, 32);
        eprint_ts();
        ...
        // bip84
        derive_bip32_cuda(&cl_state, MODE_BIP84, seed, 1, 1, child_priv);
        print_hex(child_priv, 32);
        eprint_ts();
...
void run_bip_gpu_mode_range(CudaState *cl_state, KernelMode kernel, const char *mode_name, uint32_t start_i, uint32_t end_i) {

    int BATCH_SIZE = 10000 / opts.maxkeys; if (BATCH_SIZE == 0) BATCH_SIZE = 1;
    uint8_t *entropies = (uint8_t*)malloc(32 * BATCH_SIZE * opts.maxkeys);
    uint32_t *seeds_origin = (uint32_t*)malloc(4 * BATCH_SIZE);

    uint8_t *seeds_64 = (uint8_t*)malloc(64 * BATCH_SIZE * opts.maxkeys);
    uint8_t *privs_out = (uint8_t*)malloc(32 * BATCH_SIZE * opts.maxkeys);
    ...
    for (uint32_t i = start_i; i < end_i; i++) {
        ...
        if (batch_count >= BATCH_SIZE) {
            ...
            // PBKDF2 (GPU)
            char **mnemonics_arr = (char**)malloc(sizeof(char*) * total_keys);
            for(int j=0; j<total_keys; j++) {
                mnemonics_arr[j] = (char*)malloc(512);
                memset(mnemonics_arr[j], 0, 512);
                entropy_to_mnemonic(entropies + j * 32, mnemonics_arr[j]);
            }
            derive_seed_batch_cuda(cl_state, mnemonics_arr, total_keys, seeds_64);
            for(int j=0; j<total_keys; j++) free(mnemonics_arr[j]);
            free(mnemonics_arr);

            // BIP32 Pipeline (GPU)
            derive_bip32_cuda(cl_state, kernel, seeds_64, total_keys, 1, privs_out);
...
void perf_gpu_mode() {
    ...
    uint8_t *entropies = (uint8_t*)malloc(32 * total_keys);
    uint8_t *seeds_64 = (uint8_t*)malloc(64 * total_keys);
    uint8_t *privs_out = (uint8_t*)malloc(32 * total_keys);
    ...
    derive_bip32_cuda(&cl_state, MODE_BIP49, seeds_64, total_keys, 1, privs_out);
...
int main(int argc, char **argv) {
    ...
    if (opts.gpu) {
        init_cuda(&cl_state, opts.pipeline_opt);
        eprint_ts();
        fprintf(stderr, "[GPU] CUDA acceleration enabled\n");
    }
    ...
    if (strcmp(opts.mode, "priv") == 0) {
        ...
        if (opts.gpu) {
            ...
            if (strcmp(opts.mode, "bip44") == 0) {
                kernel = MODE_BIP44; mode_label = "BIP44";
            } else if (strcmp(opts.mode, "bip49") == 0) {
                kernel = MODE_BIP49; mode_label = "BIP49";
            } else {
                kernel = MODE_BIP84; mode_label = "BIP84";
            }
