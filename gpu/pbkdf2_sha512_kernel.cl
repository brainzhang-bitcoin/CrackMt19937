...
void sha512_transform(__private ulong *state, __private const ulong *block) {
...
void hmac_sha512_prepare(__global const ulong *key, uint key_len, __private ulong *ipad_state, __private ulong *opad_state) {
    ulong k[32]; 
    ...
    if (key_len > 128) {
        ...
        uint rem = key_len - processed;
        for(int j=0; j<(rem+7)/8; j++) bl[j] = key[processed/8 + j];
        ((__private unsigned char*)bl)[rem] = 0x80;
        bl[15] = SWAP64((ulong)key_len * 8);
        sha512_transform(st, bl);
...
void hmac_sha512_step(__private const ulong *ipad_state, __private const ulong *opad_state, __private const ulong *msg, uint msg_len, __private ulong *out) {
    ulong state[8];
    for(int i=0; i<8; i++) state[i] = ipad_state[i];
    ulong block[16];
    for(int i=0; i<16; i++) block[i] = 0;
    for(int i=0; i<(msg_len+7)/8; i++) block[i] = msg[i];
    ((__private unsigned char*)block)[msg_len] = 0x80;
    block[15] = SWAP64((ulong)(1024 + msg_len * 8));
    ...
    ((__private unsigned char*)out_block)[64] = 0x80;
...
__kernel void pbkdf2_hmac_sha512(
    __global const ulong *passwords,
    __global const uint *pass_lens,
    __global const ulong *salts,
    __global const uint *salt_lens,
    uint iterations,
    __global ulong *out_seeds) {
    int gid = get_global_id(0);
    ulong ipad_state[8], opad_state[8];
    hmac_sha512_prepare(&passwords[gid * 32], pass_lens[gid], ipad_state, opad_state);
    uint s_len = salt_lens[gid];
    ulong u[8], b[32]; // Increased to 32 to support 256 byte salt
    for(int i=0; i<32; i++) b[i] = salts[gid * 32 + i];
    ((__private unsigned char*)b)[s_len] = 0x00;
    ((__private unsigned char*)b)[s_len+1] = 0x00;
    ((__private unsigned char*)b)[s_len+2] = 0x00;
    ((__private unsigned char*)b)[s_len+3] = 0x01;
